...
const LoadFundsInvoice = () => {
  ...
  return <View style={styles.container}>
      ...
      <TabView tabTitles={["Dollar", "Bitcoin", "Euro", "Pound"]} selected={selectedTab} icons={[require("./assets/dollarIcon.png"), require("./assets/bitcoinIcon.png"), require("./assets/euroIcon.png"), require("./assets/poundIcon.png")]} hideTitles={true} onPress={index => setSelectedTab(index)} style={styles.tabView} />

      <InputToken value={tokenValue} onChange={x => setTokenValue(x)} step={1} />
...
const TabView = ({
  ...
  return <View style={[tabViewStyles.paletteContainer, backgroundColorStyle, propStyle]}>
      {generator.map((item, index) => <Pressable onPress={() => onPress ? onPress(index) : null} style={index === selected ? [tabViewStyles.selected, tabColorStyle, tabViewStyles.tabItem] : [tabViewStyles.unSelected, backgroundColorStyle, tabViewStyles.tabItem]} key={index}>
